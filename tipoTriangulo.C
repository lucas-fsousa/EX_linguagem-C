...
int main(void){ // Inicio do código principal
    ...
    if(l0 + l1 > l2 && l1 + l2 > l0 && l2+l0 > l1){ // Verifica o tamanho dos lados para identificar se é possivel formar um triangulo.
        ...
        if(l0 == l1 && l1 == l2 && l2 == l0){ // Verifica se o tamanho dos lados são todos iguais, se for verdade informará ao usuario que o triangulo é equilatero
            printf("o triangulo sera equilatero!"); // Informa o resultado ao usuario
        }else if(l0 != l1 && l1 != l2 && l2 != l0){ // Verifica se todos os lados do triangulo são diferentes, se for verdadeiro informará ao usuario que o triangulo é escaleno
            printf("o triangulo sera escaleno!");
        }else{ // Se as alternativas anteriores não forem verdadeiras será informado ao usuario que o triangulo é isosceles (possui 2 lados iguais).
    ...
    return 0; // Retorna um erro caso ocorra
}
